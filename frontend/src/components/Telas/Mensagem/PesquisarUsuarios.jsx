...
export default class MostrarTodasMensagens extends React.Component {
    ...
    loadUsuarios(){
        UsuarioService.buscarUsuariosPorUsername(this.props.match.params.username, this.state.paginaAtual+1)
        .then(resp =>{
            this.setState({
                usuarios: resp.data.content,
                ultimaPagina: resp.data.last,
                primeiraPagina: resp.data.first,
                numeroUsuarios: resp.data.totalElements,
    ...
    onClickLinkHome(){
        this.setState({
            selectedContent: Selected.MENSAGENSRECEBIDAS
        })
    }
    ...
    renderUsuarios(){
        const usuarios = this.state.usuarios.map((usuario) => {
            return <div key={usuario.username}>
                <UsuarioCard
                    usuario={usuario.username}
                    nome={usuario.nome}
                    onClickMostrarUsuario={() => this.onClickMostrarUsuario(usuario.username)}
                />
            </div>
        })
        ...
        <div className="Home--content">
            ...
            <div className="page">
                <div className="pagina">
                    {this.renderInicial()} - {this.state.numeroAtual} de {this.state.numeroUsuarios} usuarios
